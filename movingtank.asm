...
;; LITTLE ENDIAN ARCHITECTURE, LOAD LO byte first, then HI byte next
...
CheckRightButton:
    ...
    lda Buttons
    and #BUTTON_RIGHT		; Check if right button is pressed (%00000001)
    beq NotRight		; If not, skip to the not-pressed logic
    	lda XVel
	bmi NotRight		; Bypassing if velocity is negative!
		clc
		adc #ACCEL		; Adding acceleration to velocity
		cmp #MAXSPEED		; Check if we reached max speed
		bcc :+
			lda #MAXSPEED	; Forcing the new velocity to be MAXSPEED
		:
	...
	bmi CheckLeftButton		; Skip if velocity is negative!
		...
		bcs :+
			lda #BRAKE+1	; Force it to BRAKE (+1 to compensate for the carry)
		:
		sbc #BRAKE		; Subtracting the brake from the velocity
		sta XVel		; Save the new updated velocity
CheckLeftButton:
    lda Buttons
    and #BUTTON_LEFT			; Checking if the left button is pressed (%00000010)
    beq NotLeft				; If not, skip to the not-pressed logic
    	lda XVel
	beq :+
	...
	cmp #256-MAXSPEED		; But prevent it from going over the max speed
	bcs :+				; If current velocity is less than the max speed, skip
		lda #256-MAXSPEED	; Force the velocity to be the max negative speed
	:
	sta XVel			; Update the X velocity's lo-byte
	jmp CheckDownButton
    NotLeft:
    	lda XVel
	bpl CheckDownButton		; If velocity is positive, skip
	cmp #256-BRAKE			; Check if the velocity can be incremented
	bcc :+				; If current velocity is less than max, skip
		lda #256-BRAKE		; Force XVel to be the brake value
	:				; If it can be incremented:
	adc #BRAKE			; Add the BRAKE to de-accelerate the negative XVel
	sta XVel			; Update the XVel lo-byte
CheckDownButton:
    ;; TODO:
    lda Buttons
    and #BUTTON_DOWN
...
DrawSpriteTile:
    ...
    lda #$18			; Adjusting the tank tiles to perform the animation
    clc
    adc TileOffset		; Add 0 or 4 to point to correct animation tile
    sta $201
